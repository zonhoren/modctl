...
> The upstream `Pull`, `Fetch`, and their Dragonfly-accelerated counterparts
...
> tag string and failed every time with `"invalid reference"` —
> `Remove` (`pkg/backend/rm.go`) already handled this correctly. Fixed here
> to check `ref.Digest()` first, falling back to `ref.Tag()`, matching
> `Remove`'s existing pattern:
>
> - `f77e496` — direct-to-registry `Pull`/`Fetch` (tag `v0.1.2-alpha.0-zonhoren.1`)
> - `7a55d32` — the Dragonfly pull/fetch path (tag `v0.1.2-alpha.0-zonhoren.2`)
>
> Both commits live on the [`zonhoren-patches`](https://github.com/zonhoren/modctl/tree/zonhoren-patches)
> branch, branched off upstream's `e592314` (Dragonfly integration). `main`
> is an untouched, Dependabot-tracked mirror of upstream and does **not**
> carry this fix — consume the `zonhoren-patches` branch or the tags above.
> This fork exists so that [`zonhoren/model-csi-driver`](https://github.com/zonhoren/model-csi-driver)
